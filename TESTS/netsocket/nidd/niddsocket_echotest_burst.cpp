...
void NIDDSOCKET_ECHOTEST_BURST()
{
    ...
    for (int i = 0; i < BURST_CNT; i++) {
        for (int x = 0; x < BURST_PKTS; x++) {
            int sent = sock.send(tx_buffers[x].payload, tx_buffers[x].len);
...
void NIDDSOCKET_ECHOTEST_BURST_NONBLOCK()
{
    CellularNonIPSocket sock;
    TEST_ASSERT_EQUAL(NSAPI_ERROR_OK, sock.open(CellularContext::get_default_nonip_instance()));
    ...
    for (int i = 0; i < BURST_CNT; i++) {
        for (int x = 0; x < BURST_PKTS; x++) {
            nsapi_size_or_error_t sent = sock.send(tx_buffers[x].payload, tx_buffers[x].len);
