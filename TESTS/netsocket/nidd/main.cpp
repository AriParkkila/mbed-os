...
void poll_pending_messages(CellularNonIPSocket &sock)
{
    uint8_t buf[1] = {0};
    sock.set_timeout(1000);
    for (int i=0; i<nidd_global::SOCKET_SEND_COUNT * 2; i++) {
        if (i == 0 || i == 2) {
...
Case cases[] = {
    ...
    //Case("NIDDSOCKET_ECHOTEST", NIDDSOCKET_ECHOTEST),
    Case("NIDDSOCKET_ECHOTEST_NONBLOCK", NIDDSOCKET_ECHOTEST_NONBLOCK),
    //Case("NIDDSOCKET_ECHOTEST_BURST_NONBLOCK", NIDDSOCKET_ECHOTEST_BURST_NONBLOCK),
    //Case("NIDDSOCKET_ECHOTEST_BURST", NIDDSOCKET_ECHOTEST_BURST),
