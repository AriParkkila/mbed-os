...
nsapi_error_t AT_CellularNetwork::delete_context(int cid)
{
    _at.lock();
...
nsapi_error_t AT_CellularNetwork::connect()
{
    ...
    if (err != NSAPI_ERROR_OK) {
        ...
        // If new PDP context created and failed to activate, delete it
        if (_new_context_set) {
            delete_context(_cid);
    ...
    while (_at.info_resp()) {
        ...
        if (pdp_type_len > 0) {
            ...
            if (apn_len >= 0) {
                ...
                if (pdp_stack != DEFAULT_STACK && get_modem_stack_type(pdp_stack)) {
